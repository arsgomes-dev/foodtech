...
function loadOccupations(code, occupation) {
    ...
    var data = post;
    $.post(dir + "/list/Occupations/selectProfile", data, function (response) {
        $('#ocuppationEdit_div').html(response);
    });
}
$(document).ready(function () {
    //Datemask dd/mm/yyyy
...
function update(form) {
    ...
    if (validationForm(form) && emailSearchProfile(form.email) === true && cpfSearchProfile(form.cpf) === true) {
        ...
        $.post(dir + "/control/Profile/Save", data, function (response) {
...
function emailSearchProfile(input) {
    ...
    if (emailValidation(input)) {
        ...
        $.post(dir + "/list/Profile/getEmail", "email=" + input.value, function (response) {
            if (response === "1") {
...
function cpfSearchProfile(input) {
    ...
    if (cpfValidate(input)) {
        ...
        $.post(dir + "/list/Profile/getCpf", posts, function (response) {
            if (response === "1") {
                input.classList.add('is-invalid');
...
function profile_wallpaper_save() {
    if (document.querySelector("#dir_site")) {
        var dir_site = document.querySelector("#dir_site");
        var dir = "";
        if (dir_site.value !== null && dir_site.value !== "") {
    ...
    var forms = document.getElementById('form_wallpaper_profile');/*$('#form_wallpaper_profile')[0];*/
    var form_d = new FormData(forms);
    $.ajax({
        url: dir + "/control/Profile/Wallpaper",
        type: 'POST',
        enctype: 'multipart/form-data',
        data: form_d,
        processData: false, // tell jQuery not to process the data
...
function readURLWallpaper(input) {
    if (input.files && input.files[0]) {
        ...
        reader.onload = function (e) {
            document.getElementById('div_modal_wallpaper_img').style.display = 'block';
            document.getElementById('div_modal_wallpaper_i_div').style.display = 'none';
            $('#div_modal_wallpaper_img').attr('src', e.target.result);
        }
...
$(document).ready(function () {
    $('#profile_wallpaper').on("change", function () {
        readURLWallpaper(this);
    });
});
function upload_wallpaper() {
    $('#profile_wallpaper').trigger('click');
}
function cleanForm_wallpaper() {
    document.getElementById('div_modal_wallpaper_img').style.display = 'none';
    document.getElementById('div_modal_wallpaper_i_div').style.display = 'block';
    $('#div_modal_wallpaper_img').attr('src', '');
    $('#profile_wallpaper').attr('value', '');
}
...
function passwSave() {
    ...
    if (passVerify() === true) {
        ...
        $.post(dir + "/control/Profile/UpdatePwd", data, function (response) {
            var msg = response.split("->");
            if (msg[0] === "1") {
                Toast.fire({
                    icon: 'success',
...
/*$(function () {
    $("#passNew").complexify({}, function (valid, complexity) {
        document.getElementById("mtSenha").value = complexity;
    });
    vai no html em baixo da senha <meter value="0" id="mtSenha" max="100"></meter>
