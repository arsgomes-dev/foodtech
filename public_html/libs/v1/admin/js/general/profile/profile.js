...
function emailSearchProfile(input) {
    ...
    if (emailValidation(input)) {
        ...
        $.post(dir + "/profile/getemail", "email=" + input.value, function (response) {
            if (response === "1") {
...
function cpfSearchProfile(input) {
    ...
    if (cpfValidate(input)) {
        ...
        $.post(dir + "/profile/getcpf", posts, function (response) {
            if (response === "1") {
                input.classList.add('is-invalid');
...
function passwSave() {
    ...
    if (passVerify() === true) {
        ...
        $.post(dir + "/profile/updatepwd", data, function (response) {
            var msg = response.split("->");
            if (msg[0] === "1") {
                Toast.fire({
                    icon: 'success',
