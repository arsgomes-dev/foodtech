...
namespace Microfw\Src\Main\Common\Helpers\Public\UploadFile;

use Microfw\Src\Main\Common\Entity\Public\Returning;
use Microfw\Src\Main\Common\Helpers\Public\Translate\Translate;
use Microfw\Src\Main\Common\Helpers\Public\TreatName\TreatImageName;
...
class UploadImg {
            ...
            if ($isFile) {
                ...
                if ($img['size'] <= $MAXIMUM_FILESIZE && preg_match($rEFileTypes, strrchr($safe_filename, '.'))) {
                    ...
                    if ($isMove) {
                        ...
                        $returning->setValue(2);
                        $returning->setDescription(ucfirst($translate->translate('Erro ao fazer upload da imagem!', $_SESSION['client_lang'])));
                    }
                } else {
                    $returning->setValue(2);
                    $returning->setDescription(ucfirst($translate->translate('O tamanho da imagem excede o tamanho permitido de 5mb!', $_SESSION['client_lang'])));
                }
            }
        } else {
            $returning->setValue(2);
            $returning->setDescription(ucfirst($translate->translate('Ocorreu um erro no upload da imagem!', $_SESSION['client_lang'])));
        }
        return $returning;
    }
    ...
    public function upload($dir_base, $input_name, $img, $name_file = null) {
        ...
        if ($treatImageName->strposa($name, $filtro)) {
            ...
            $returning->setValue(2);
            $returning->setDescription(ucfirst($translate->translate('O formato da imagem é inválido! Formatos aceitos: Gif, Jpeg, Jpg e Png!', $_SESSION['client_lang'])));
            return $returning;
        }
    }
    ...
    public function uploadFiles($dir_base, $input_name, $img, $name_file = null) {
        ...
        if ($treatImageName->strposa($name, $filtro)) {
            ...
            $returning->setValue(2);
            $returning->setDescription(ucfirst($translate->translate('O formato da imagem é inválido! Formatos aceitos: PDF e XML!', $_SESSION['client_lang'])));
            return $returning;
        }
    }
public function uploadAll($dir_base, $input_name, $img, $name_file = null) {
        ...
        if ($treatImageName->strposa($name, $filtro)) {
            ...
            $returning->setValue(2);
            $returning->setDescription(ucfirst($translate->translate('O formato da imagem é inválido! Formatos aceitos: Gif, Jpeg, Jpg, Png e PDF!', $_SESSION['client_lang'])));
            return $returning;
        }
    }
    public function delete($dir_base, $file) {
        ...
        if (file_exists($dir_base . $file)) {
            ...
            $returning->setValue(1);
            $returning->setDescription(ucfirst($translate->translate('Arquivo excluído com sucesso!', $_SESSION['client_lang'])));
            return $returning;
        } else {
            //erro ao excluir
            $returning->setValue(2);
            $returning->setDescription(ucfirst($translate->translate('Erro ao excluir arquivo!', $_SESSION['client_lang'])));
            return $returning;
        }
    }
