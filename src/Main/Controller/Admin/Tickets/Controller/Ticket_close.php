...
if (in_array("ticket_close", $privilege_types)) {
    //verifica se as variáveis enviadas via POST estão preenchidas, caso não retorna um ERRO
    if (!empty(array_filter($_POST)) && $_POST) {
        if (!empty($_POST['code']) && $_POST['code'] && !empty($_POST['description_close']) && $_POST['description_close']) {
            ...
            //Define a data e hora do encerramento do ticket e envio do e-mail
            ...
            if ($_POST['stTicket'] === "" || $_POST['stTicket'] === null || empty($_POST['stTicket']) || !isset($_POST['stTicket'])) {
                $ticket->setStatus(2);
            } else {
                $ticket->setStatus($_POST['stTicket']);
            }
            $ticket->setClosure_description($_POST['description_close']);
            $ticket->setDate_closing($dataI);
            ...
            $notification = $notification->getQuery(single: true, customWhere: [['column' => 'description_type', 'value' => "ticket_closed"]]);
           if ($notification !== null) {
                ...
                if ($notification->getStatus() === 1) {
                    ...
                    $pattern = array('{{{ticket.title}}}', '{{{ticket.description}}}', '{{{ticket.dateClosing}}}', '{{{ticket.hourClosing}}}', '{{{ticket.dateSend}}}', '{{{ticket.hourSend}}}', '{{{website.title}}}', '{{{website.http}}}');
            ...
            if ($return == 1) {
                echo "1->" . $translate->translate('Ticket encerrado com sucesso!', $_SESSION['user_lang']);
            } else {
                echo "2->" . $translate->translate('Erro ao encerrar ticket!', $_SESSION['user_lang']);
            }
        } else {
    ...
    echo "2->" . $translate->translate('Você não possui permissão para encerrar o Ticket!', $_SESSION['user_lang']);
}    