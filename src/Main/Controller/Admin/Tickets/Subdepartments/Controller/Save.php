...
//Função para SALVAR/ATUALIZAR subdepartamentos de atendimento do TICKET
...
if (in_array("ticket_department_create", $privilege_types)) {
    ...
    if (!empty(array_filter($_POST)) && $_POST) {
        if (!empty($_POST['title']) && isset($_POST['title']) && !empty($_POST['status_priority']) && isset($_POST['status_priority']) && !empty($_POST['department']) && isset($_POST['department'])) {
            //Declara a classe e seta as informações
            $ticket = new TicketDepartmentSubdepartment;
            if (!empty($_POST['code']) && isset($_POST['code'])) {
                ...
                //Informa a qual departamento o subdepartamento pertence
                $ticket->setTicket_department_id($_POST['department']);
            }
            ...
            if (!empty($_POST['status_priority'])) {
                if (isset($_POST['status_priority'])) {
                    $ticket->setTicket_department_subdepartment_priority_id($_POST['status_priority']);
                }
            }
